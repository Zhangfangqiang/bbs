    ...
    <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
      <div class="card">
        <div class="card-header">
          <h4>
            <i class="glyphicon glyphicon-edit"></i> 编辑内容
          </h4>
        </div>
        ...
        <div class="card-body">
          ...
          <form action="{{route('web.contents.update',$content->id)}}" method="POST" accept-charset="UTF-8" enctype="multipart/form-data">
            <div class="form-group">
              <label for="title-field">标题</label>
              <input id="title-field" name="title" type="text" value="{{old('title') ?? $content->title }}" class="form-control"/>
            </div>

            <div class="form-group">
              <label for="category-name-field">分类</label>
              <input id="category-name-field" name="c_title" type="text" value="{{old('c_title') ?? $content->category()->get()->pluck('name')->implode(',') }}" class="form-control" readonly onclick="doSelectCategory()"/>
              <input id="category-id-field"   name="c_id"    type="text" value="{{old('c_id')    ?? $content->category()->get()->pluck('id')->implode(',') }}"    class="form-control" required hidden>
            </div>

            <div class="form-group">
              <label for="content-field">文本内容</label>
              <script id="content-field" style="height: 600px" name="content" type="text/plain">{!! old('content') ?? $content->content !!}</script>
            </div>
